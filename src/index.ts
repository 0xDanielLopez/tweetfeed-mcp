		...
		inputSchema: {
			...
			properties: {
				...
				tag: {
					type: "string",
					description:
						"Optional: filter by tag, case-insensitive substring match. Examples: 'phishing', 'cobaltstrike', 'ransomware', 'APT', 'Lockbit'. 93 tags exist - see https://tweetfeed.live/ for the live taxonomy.",
				},
				type: {
					type: "string",
		...
		inputSchema: {
			...
			properties: {
				tag: {
					type: "string",
					description:
						"Tag to look up (e.g. 'phishing', 'CobaltStrike', 'lockbit'). Case-insensitive. The leading '#' is optional. 93 tags exist - see https://tweetfeed.live/tags/ for the full list.",
				},
				limit: {
					type: "number",
		...
		inputSchema: {
			...
			properties: {
				brand: {
					type: "string",
					description:
						"Optional: filter by targeted brand, case-insensitive substring match against targeted_brand (e.g. 'paypal', 'microsoft'). Campaigns with no identified brand are excluded when this is set.",
				},
				min_confidence: {
					type: "string",
					enum: ["low", "medium", "high"],
					description:
						"Optional: minimum clustering confidence to include (low < medium < high). Only campaigns at or above this confidence are returned.",
				},
				limit: {
					type: "number",
...
// Shape of GET /v1/ioc?value=<v> - the exact 365-day sharded-index lookup.
...
// on any non-2xx response or network failure so callers can fall back to the
// 30-day scan instead of erroring the whole tool call out.
async function fetchIocLookup(env: Env, value: string): Promise<IocLookupResult | null> {
	const url = `${API_BASE}/v1/ioc?value=${encodeURIComponent(value)}`;
	try {
		const r = await env.API.fetch(new Request(url, { headers: { "User-Agent": UA } }));
		if (!r.ok) return null;
		return (await r.json()) as IocLookupResult;
	} catch {
		return null;
	}
}
...
async function toolCheckIp(env: Env, args: Record<string, unknown>) {
	...
	// Exact 365-day lookup first (same sharded index as enrich_ioc). Falls
	// back to the 30-day substring scan on a lookup miss or if /v1/ioc itself
	// is unreachable (non-2xx/timeout).
	const lookup = await fetchIocLookup(env, needle);
	if (lookup?.found && Array.isArray(lookup.records) && lookup.records.length > 0) {
		return textContent(
			`Exact match for IP "${needle}" in the past 365 days of TweetFeed (query normalized to "${lookup.query}"):\n\n` +
				JSON.stringify(lookup.records, null, 2),
		);
	}
	...
	if (matches.length === 0) {
		return textContent(
			`IP "${needle}": ${yearVerdict}; NOT found by substring in the last 30 days of TweetFeed (${rows.length} IP IOCs scanned).`,
		);
	}
	const preview = matches.slice(0, 50);
	return textContent(
		`IP "${needle}": ${yearVerdict}; found ${matches.length} substring match(es) in the last 30 days of TweetFeed${matches.length > preview.length ? ` (showing first ${preview.length})` : ""}:\n\n` +
			JSON.stringify(preview, null, 2),
	);
}
...
async function toolCheckHash(env: Env, args: Record<string, unknown>) {
	...
	// Exact 365-day lookup first (same sharded index as enrich_ioc). Falls
	// back to the 30-day exact scan on a lookup miss or if /v1/ioc itself is
	// unreachable (non-2xx/timeout).
	const lookup = await fetchIocLookup(env, raw);
	if (lookup?.found && Array.isArray(lookup.records) && lookup.records.length > 0) {
		return textContent(
			`Exact match for ${hashType.toUpperCase()} hash "${raw}" in the past 365 days of TweetFeed (query normalized to "${lookup.query}"):\n\n` +
				JSON.stringify(lookup.records, null, 2),
		);
	}
	...
	if (matches.length === 0) {
		return textContent(
			`${hashType.toUpperCase()} hash "${raw}": ${yearVerdict}; NOT found in the last 30 days of TweetFeed (${rows.length} ${hashType.toUpperCase()} IOCs scanned).`,
		);
	}
	return textContent(
		`${hashType.toUpperCase()} hash "${raw}": ${yearVerdict}; found ${matches.length} match(es) in the last 30 days of TweetFeed:\n\n` +
...
// Routed through env.API (the api-worker service binding) rather than a raw
// fetch to raw.githubusercontent.com: that host is a single Fastly POP once
// pinned, so a stale/edge-cached counts.json could linger for hours (see
// CLAUDE.md raw GH POP-pinning gotcha). /v1/counts is api-worker's own
// passthrough of the same JSON, cached the way api-worker already caches
// everything else this MCP fetches through the binding.
async function fetchCountsJson(env: Env): Promise<{
...
async function toolGetTrending(env: Env, args: Record<string, unknown>) {
...
function describeCampaignFilters(brand: string, minConfidence: string): string {
	const parts: string[] = [];
	if (brand) parts.push(`targeted_brand~="${brand}"`);
	if (minConfidence) parts.push(`min_confidence=${minConfidence}`);
	return parts.join(", ");
}
...
async function toolGetCampaigns(env: Env, args: Record<string, unknown>) {
	...
	if (minConfidence && !(minConfidence in CONFIDENCE_RANK)) {
		throw {
			code: ERR.INVALID_PARAMS,
	...
	const allCampaigns = doc.campaigns ?? [];
	let campaigns = allCampaigns;

	if (brand) {
		campaigns = campaigns.filter(
			(c) => typeof c.targeted_brand === "string" && c.targeted_brand.toLowerCase().includes(brand),
		);
	}
	if (minConfidence) {
		const minRank = CONFIDENCE_RANK[minConfidence];
		campaigns = campaigns.filter((c) => {
			const rank = typeof c.confidence === "string" ? CONFIDENCE_RANK[c.confidence.toLowerCase()] : undefined;
			return rank !== undefined && rank >= minRank;
		});
	}

	const totalMatched = campaigns.length;
	if (totalMatched === 0) {
		const filterDesc = describeCampaignFilters(brand, minConfidence);
		if (filterDesc && allCampaigns.length > 0) {
			return textContent(
				`No campaigns matched ${filterDesc} (${allCampaigns.length} total campaign(s) tracked this window).`,
			);
		}
		return textContent(
		...
		JSON.stringify(
			{
				...
				stale_since: doc.stale_since ?? null,
				campaign_count_total: doc.campaign_count ?? allCampaigns.length,
				campaign_count_returned: out.length,
				campaigns: out,
			},
			null,
			2,
